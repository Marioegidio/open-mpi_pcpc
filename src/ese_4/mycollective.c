...
void myBroadcast(void *buf,int bufSize,MPI_Datatype datatype, int source, MPI_Comm comm, int tag){
    ...
    if( rank == source )
        ...
        MPI_Recv(buf, bufSize, datatype, source, tag, comm, &Stat);
}
...
void myGather(void *buf,int bufSize,MPI_Datatype datatype, int root, MPI_Comm comm, void *returnBuffer, int *returnSize){
    ...
    if( rank == root ){
        ...
        for(int i = 0; i < numtasks; i++){
            if(i != root){
                MPI_Recv(returnBuffer+(currentSender*dataSize),bufSize,datatype,i,123,MPI_COMM_WORLD,&status);
                *returnSize = *returnSize + bufSize;
                currentSender++;
            }
        }
    }else{
        MPI_Send(buf, bufSize, datatype, root, 123, MPI_COMM_WORLD);
    }
}
...
void myReduce(int array[],int bufSize, int dest, MPI_Comm comm, int mode, int *result){
    ...
    myScatter(array,bufSize,MPI_INT,dest,MPI_COMM_WORLD,arrayRes, &size);
